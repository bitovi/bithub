require 'rails_helper'

RSpec.describe Bit, :type => :model do

  context "upon creation" do

    describe "#save" do
      it "raises an error on save! b/c there is no feed / tags / rules applied" do
        generic_bit = FactoryGirl.build(:bit)
        expect{generic_bit.save!}.to raise_error(StandardError)
      end
    end

    describe "#thread" do
      it "fetches the bit itself wrapped in an array if there is no thread" do
        e = FactoryGirl.create(:github_issue, title: "Why is this happening?")
        expect(e.thread).to include(e)
      end

      it "fetches the whole thread" do
        pe = FactoryGirl.create(:github_issue, title: "Why is this happening?")
        ce1 = FactoryGirl.create(:github_issue_comment, title: "I don't care.", parent: pe)
        ce2 = FactoryGirl.create(:github_issue_comment, title: "Wat? Qua?", parent: pe)
        expect(pe.thread).to match_array(ce1.thread)
        expect(pe.thread.length).to eql(3)
      end
    end

    describe "#bump_thread" do
      it "updates the thread_updated_ts attribute for all bits in a thread" do
        pe = FactoryGirl.create(:github_issue, title: "Why is this happening?", origin_ts: Time.now+5)
        ce1 = FactoryGirl.create(:github_issue_comment, title: "I don't care.", parent: pe, origin_ts: Time.now+10)
        ce2 = FactoryGirl.create(:github_issue_comment, title: "Wat? Qua?", parent: pe, origin_ts: Time.now+15)

        ce2.bump_thread
        expect(pe.reload.thread_updated_ts).to be > pe.origin_ts
        expect(ce1.reload.thread_updated_ts).to be > ce1.origin_ts
        expect(ce2.reload.thread_updated_ts).to eq(ce2.origin_ts)
      end
    end

    describe "#cache_key" do
      it "uses the id, updated_at and thread_updated_ts timestamps when they are present" do
        bit = FactoryGirl.create(:twitter_tweet, title: "Bit in bit_spec, testing #cache_key")
        expect(bit.cache_key).to eq "bits/#{bit.id}-#{bit.updated_at.utc.to_s(:number)}-#{bit.thread_updated_ts.utc.to_s(:number)}"
      end
    end
  end
end
