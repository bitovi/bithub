...
describe DummySet do
  ...
  describe "#add" do
    it "responds with 'false' when trying to add an already present elem" do
      set = DummySet.new
      expect(set.add(data_set.first)).to be_true
      expect(set.add(data_set.first)).to be_false
    end
  end
