...
RSpec.describe Wrappers::Meetup::Venue, :type => :domain do
  ...
  subject(:venue) do
    Wrappers::Meetup::Venue.new(raw_venue)
  end
  ...
  describe "#composite_location" do
    it "should construct the composite_location from name, address, city and country" do
      expect(venue.composite_location).to eq "#{venue.name}, #{venue.address}, #{venue.city}, #{venue.country}"
    end
  end
