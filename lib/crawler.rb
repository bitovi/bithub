...
def start_crawler
  AMQP.start($mq_cs) do |connection, open_ok|
    ...
    exchange = channel.fanout("testy", :auto_delete => true)
    ...
    EM.add_periodic_timer(10, &Handler::Disqus.handler($log, exchange))

    $log.info "Registering Forums"
    EM.add_periodic_timer(15, &Handler::Forums.handler($log, exchange))

    $log.info "Registering Blog"
    EM.add_periodic_timer(20, &Handler::Blog.handler($log, exchange))
  end
end
