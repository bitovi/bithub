...
import fixtureData from "./fixture_data.json";
...
QUnit.test("Card is correctly rendered", function(){
	renderTemplate({
...
QUnit.test("Sharing panel is expanded on click", 3, function(){
	var bit =  Bit.model(fixtureData.data[0]);
	renderTemplate({
		bit: bit,
		state: getState(false)
	});
	...
	F.wait(1, function(){
		ok(F('bh-bit').scope().attr('sharePanelOpen'), 'VM is updated with share panel attr');
	});
});
