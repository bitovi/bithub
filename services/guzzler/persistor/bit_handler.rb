...
module Guzzler::Persistor
  ...
  class BitHandler < Handler
    ...
    def process(packet)
      ...
      Guzzler.zadd('bit_retries_q', Time.now.to_f, {
    ...
    def run_pipeline(event)
      bit = Bits.bit_instance(event.wrapped)

      bit_processing_time = Benchmark.measure do
        bit.procure.update_if_found.group.normalize.persist!.route
      end
      
      event.update_attribute(:is_processed, true)

      Guzzler.logger.info "[#{name_for_logs}][#{bit.repr_for_logs}] processed in #{bit_processing_time}"
      bit
    ensure
