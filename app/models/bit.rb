class Bit < ActiveRecord::Base
  ...
  has_many :hubs, through: :moderations

  has_many :service_bits, dependent: :destroy
  has_many :services, through: :service_bits

  has_many :ownerships, foreign_key: :bit_id, dependent: :destroy
  has_many :owners, through: :ownerships, source: :owner
  has_many :hosts, through: :ownerships, source: :host

  belongs_to :parent, :class_name => "Bit"
  has_many :children, :foreign_key => "parent_id", :class_name => "Bit"
  ...
  scope :belong_to_a_thread, -> do
    where 'parent_id IS NOT NULL OR id IN (SELECT parent_id from bits)'
  end

  scope :have_no_thread, -> do
    where 'parent_id IS NULL AND id NOT IN (SELECT parent_id from bits)'
  end

  # Parent/child
  scope :only_parents, -> { where('id IN (SELECT parent_id from bits WHERE parent_id IS NOT NULL)') }
  scope :only_children, -> { where('parent_id IS NOT NULL') }
  scope :no_parents, -> { where('id NOT IN (SELECT parent_id FROM bits WHERE parent_id IS NOT NULL)') }
  ...
  def self.by_service(service_id)
    joins(:service_bits).where("service_bits.service_id" => service_id)
  end
  ...
  def is_approved(hub = nil)
    # Used when bits are decorated with attributes from moderations,
    ...
    # If bit doesn't have these attributes (is_approved_*), then given an hub,
    # find the appropriate moderations record and read that info from it
    else
      return nil if hub.nil?
      memoize('is_approved', hub.id) do
        moderations.find_by_hub_id(hub.id).is_approved
      end
    end
  end

  # See first 5 lines of ModerationsController#build_scope method
  def is_pinned(hub = nil)
    if has_attribute?(:is_pinned)
      read_attribute(:is_pinned)
    else
      return nil if hub.nil?
      memoize('is_pinned', hub.id) do
        moderations.where(:hub_id => hub.id).first.is_pinned?
      end
    end
  end
  
  # See first 5 lines of ModerationsController#build_scope method
  def decision(hub = nil)
    if has_attribute?(:decision)
      read_attribute(:decision)
    else
      return nil if hub.nil?
      memoize('decision', hub.id) do
        moderations.where(:hub_id => hub.id).first.decision
      end
    end
  end
  ...
  def children_with_includes
    children.merge(Bit.scoped_with_includes)
  end

  def thread
    if parent_id # When an event is a child
      Bit.where('id = ? OR parent_id = ?', parent_id, parent_id)
    else # When an event is a parent
      Bit.where('id = ? OR parent_id = ?', id, id)
    end
  end
  ...
  # FIXME dirty hack, should be delegated to a type in Bits::{FEED}::{TYPE}
  def bump_thread
  ...
  def wrapper_class
    ...
    if ::Bits.constants.include?(fn) && ::Bits.const_get(fn).constants.include?(tn)
      ::Bits.const_get(fn).const_get(tn)
    end
  end
