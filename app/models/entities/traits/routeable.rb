  ...
  module Routable
    ...
    def route_to_embed
      if embed_id && (embed = Embed.find_by_id(embed_id))
        if (link = embed.make_link_to(@instance))
          Notifier.notify_client(:entity_moderated, { entity: @instance })
        end
      end
    end
    ...
    def route_to_service
      if service_id && (service = Service.find_by_id(service_id))
        if (link = service.make_link_to(@instance))
          ...
          Notifier.notify_client(:entity_routed_to_service, { entity: @instance, service: service })
        end
      end
    end
  end
