class BitDecorator < Draper::Decorator
  delegate_all
  ...
  def is_approved
    current_hub = context.fetch(:hub)
    source.is_approved(current_hub)
  end

  def is_pinned
    current_hub = context.fetch(:hub)
    source.is_pinned(current_hub)
  end
  
  def decision
    current_hub = context.fetch(:hub)
    source.decision(current_hub)
  end

  def images
    BitImagesMapper.new(source).build
  end
